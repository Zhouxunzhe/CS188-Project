...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        ...
        for uv in uncon_var:
            unconditioned_variables.append(uv)
        for cv in con_var:
            if cv not in conditioned_variables:
                conditioned_variables.append(cv)
    conditioned_variables = [cv for cv in conditioned_variables if cv not in unconditioned_variables]
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for assignment in assignments:
            ...
            new_factor.setProbability(assignment, new_prob+prob)
