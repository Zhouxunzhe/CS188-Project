...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # get all factors relative to evidence
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminate_variable in eliminationOrder:
            factors, joined_factor = joinFactorsByVariable(factors, eliminate_variable)
            # eliminate
            if len(joined_factor.unconditionedVariables()) > 1:
                new_factor = eliminate(joined_factor, eliminate_variable)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # P(ghostPosition|observation, pacmanPosition) = P(observation|ghostPosition)*P(ghostPosition) / P(observation)
        # P(ghostPosition) = P(observation) = 1
        pacmanPosition = gameState.getPacmanPosition()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        new_beliefs = {key: 0 for key in self.allPositions}
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos, prob in newPosDist.items():
                # prob = P(G_t+1|G_t, gameState)
                # P(G_t+1|gameState) = sum(P(G_t+1|gameState) * P(G_t+1|G_t)))
                # P(G_t + 1|G_t) = P(G_t+1|G_t, gameState)
                new_beliefs[newPos] += self.beliefs[oldPos] * prob
        for key in new_beliefs:
            self.beliefs[key] = new_beliefs[key]
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # weight = P(observation|particlePosition, pacmanPosition)
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        new_beliefs = {key: 0 for key in self.particles}
        for i in range(self.numParticles):
